...
const kittyPrompts = {
	orangePetNames(kittyData) {
		...
		// Annotation:
		// return a smaller subset of an array (.filter())
		// then return just the NAMES of the orange cats (.map())
	},
	...
	sortByAge(kitties) {
		...
		/* CODE GOES HERE */
		const sortedKitties = kitties.sort((x, y) => {
			return y.age - x.age;
		});
	...
	growUp(kitties) {
		...
		const kittyKeys = Object.keys(kitties);

		const grownKitties = kittyKeys.map((i) => {
			kitties[i].age = kitties[i].age + 2;
			return kitties[i];
		});
		...
		// Annotation:
		// an array on objects
		// need to make the objects iterable => Object.keys()
		// Note: when you use Object.keys with an IM,
		// name the param index or something so you don't forget what it is
	},
};
...
const breweryPrompts = {
	getBeerCount() {
		...
		// 40
		/* CODE GOES HERE */
		const getTotal = breweries.reduce((accum, bar) => {
			accum += bar.beers.length;
			return accum;
		}, 0);
	...
	getBreweryBeerCount() {
		...
		/* CODE GOES HERE */
		const addBeerCount = breweries.map((bar) => {
			const newList = {};
			newList.name = bar.name;
			newList.beerCount = bar.beers.length;
			newArray.unshift(newList);
			return newList;
		});

		return addBeerCount;
		// Annotation:
		// we want to create a new object with just the name property and create a beerCount property
	},
	...
	getSingleBreweryBeerCount(breweryName) {
		...
		const getBeer = breweries.find((bar) => {
			return bar.name === breweryName;
		});
